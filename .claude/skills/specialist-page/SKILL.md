...
- (13.09, Галкин + студия декора) Кириллические пути в аргументах node через
  Git Bash доезжают битыми → API отвечает 404. Пути папок диска — в JSON-файл
  (`paths.json`), не в argv. Общие скрипты превью/оригиналов —
  `research/specialists/_prev.mjs`, `_pick.mjs`.
- Каталог ведущих (`vedushchie/index.astro`) — своя страница и раньше читала
  только демо-массив `VEDUSHCHIE`: JSON-ведущий в сетку не попадал. Теперь
  читает `SPECIALISTS_BY_CATEGORY.vedushchie`, у JSON берёт готовый
  `s.filters`, без `priceFrom` пишет «Цена по запросу». Для ведущего в JSON
  заполнять и `filters` (для сетки), и прямые поля (`age`, `hasOwnDJ`… — для визитки).
- `verify` искал числа из бакетов фильтров («35–50», «15+») в сырье и ронял
  карточку — поля `filters/age/cities/styles/formats/languages` из проверки
  чисел исключены.
- Кадры уже 1200 px в манифесте `photos.json` встречаются часто (скрины,
  webp из соцсетей) — после `prep-photos` пробежать по `width` и выкинуть,
  контактный лист по превью их не выдаёт.
- Портрет на аватар у декоратора нашёлся, но 667×1000: на аватар хватает
  (кроп 500 px без апскейла), в блок «О нас» (`portrait`) — нет, поле не ставим.
- (14.09, видео) Ролики с диска заводятся тем же заходом: листинг
  `_vid.mjs list`, качаем 2–3 самых лёгких `.mov` (у фотографа 36–47 МБ)
  или промо ведущего; полоска кадров `fps=1/4 … tile=` одним Read, чтобы
  выбрать 8 секунд; команда пережатия из VIDEO.md **со звуком**
  (`-c:a aac -b:a 96k`, `sound: true`). Горизонтальное промо режется
  центром `crop=ih*2/3:ih` — куски с титрами на кадре не брать. Запись —
...
- Название подрядчика часто лежит в имени корневой папки диска
  (`d.name` в ответе API: «Meriyadecor (Евгению)» = Maria Decor) — смотреть
  его первым делом и писать в notes.md, а не заводить карточку «без имени».
