...
- качать папку диска целиком (у подрядчиков 1–4 ГБ: превью → отбор → **50 оригиналов**;
  норма заказчика 17.09.2026, глобально на весь каталог: «до 50 фото всем
  площадкам и подрядчикам». Блок «Фотографии» внизу визитки снят (#27, 23.09):
  все `photos` и ролики листаются в первом экране (`HeroStage`), так что
  порядок `photos` — это порядок листания, `[0]` видят первым.
  Меньше 50 — только если годных кадров
  на диске нет: сжатые под веб < 1200 px и битые по-прежнему не берём);
...
**Максимум 5 роликов на подрядчика** (заказчик, 24.09.2026). Исключение —
Reels-мейкеры (`rils-meikery`): у них ролик и есть работа, им больше
(25.09.2026; точное число уточнить у заказчика, если не названо). Отбор — по обложкам роликов (поле
`preview` в листинге публичного ресурса API), качаем только выбранные,
весь набор не тянем. У фотографа обычно 2–3 самых лёгких файла (36–47 МБ)
или промо ведущего; гигабайтные исходники не качать. Пережатие — команда из VIDEO.md **со звуком**
(`-c:a aac -b:a 96k`, в данных `sound: true`), до 60 секунд (короче — целиком),
720×1080, до 8 МБ. Точку старта и исходник — в `research/reel-map.json`.
...
- `photos` — до 50 работ (норма — 50, см. выше), `[0]` главный (портрет-ориентация, первый экран 4:5).
...
- (24.09, Косарева) `verify-specialist` искал наборы фильтров текстом
  в `specialists.ts`, а они переехали в `specialist-filters.ts` (хелперы
  multi/single) — любая новая категория падала «нет фильтров». Теперь
  скрипт импортирует `FILTERS_BY_CATEGORY` напрямую.
- (24.09) Ролики до 60 с: PocketBase держал потолок файла ролика 4 МБ —
  миграция `1757700400_reels_60s` подняла до 10 МБ. Перегнали ролик тем
  же именем — `pb-seed-specialists <slug> --refresh-reels`, иначе заливка
  пропускает его как уже залитый.
- (24.09) Визитка подписывала любую `priceFrom` «за 6 часов работы». Цена
  за другую единицу (час у Reels-мейкера) — пишем её в `priceNote`
  («за час съёмки»), визитка ставит его под цену.
- (24.09, Балашова) Портфолио нет, только закрытые соцсети — по решению
  заказчика заводим БЕЗ ФОТО: `photos: []`, первый экран (`HeroStage`)
