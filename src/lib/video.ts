...
//   • УШЛО С ЭКРАНА — ОТПУСКАЕМ И САМ ФАЙЛ (src снимается). Паузы мало:
//     у браузера конечное число видео-декодеров, а в ленте роликов
//     несколько десятков. Пока пролистанные ролики держали свой src,
//     они держали и декодер — и у тех, до кого человек долистывал
//     позже, play() просто отклонялся. Со стороны это выглядело как
//     «у одних карточек видео идёт, у других нет», без всякой системы.
//     Возврат бесплатный: файл к этому моменту лежит в кэше;
...
export function bindVideo(
  ...
  const sync = () => {
    ...
    if (onScreen && active()) {
      ...
      // Кадр ушёл с экрана — отдаём декодер (см. шапку файла). Ролик,
      // которому человек сам включил звук, не трогаем: он звучит, пока
      // его не заглушит своя отметка видимости в bindSoundButtons.
      if (!onScreen && video.muted) {
