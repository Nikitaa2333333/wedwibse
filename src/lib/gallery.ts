...
// подрядчиков) сумма растёт линейно — 168 <img> в HTML главной при нынешнем
// размере каталога, и дальше только больше при каждой новой карточке.
// Резать нужно уже ПЕРЕМЕШАННЫЙ поток (после interleave/blendReels и
// вставки статей), а не «первые N по порядку источников»: VENUES в массиве
// идут первыми, и такой срез оставил бы в первом экране одни площадки,
// целиком выкинув подрядчиков, ролики и статьи. Значение — на 3–4 экрана
// прокрутки: 2 колонки на телефоне, 4–5 на ПК (~1280 CSS-px при масштабе
// 150%, см. CLAUDE.md).
const BOARD_TILE_LIMIT = 44;
