...
REF = {
    ...
    "Шоссе": ["в Москве", "Ленинградское", "Новорижское", "Волоколамское", "Пятницкое", "Ильинское", "Рублёво-Успенское",
              "Минское", "Можайское", "Киевское", "Боровское", "Калужское", "Варшавское", "Симферопольское",
              "Каширское", "Новорязанское", "Егорьевское", "Носовихинское", "Горьковское", "Щёлковское",
              "Ярославское", "Дмитровское", "Осташковское"],
...
def build_sheet(title, cols, rows, max_rows=300):
    ...
    for i, (key, label, group, kind, ex) in enumerate(cols, start=1):
        ...
        if kind == "yes":
            ...
            # для multi — список на месте как подсказка; в Google Таблице включить «несколько вариантов»
            dv = DataValidation(type="list", formula1=ref_range(kind.split(":", 1)[1]), allow_blank=True, showErrorMessage=False)
        elif kind == "int":
